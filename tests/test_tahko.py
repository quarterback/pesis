...
from pesis import db, demo, ingest, tahko

SPEC = tahko.StatSpec("kl_pct", "karkilyonnit", "karki_yritykset",
...
def test_empty_history_returns_league_mean():
    proj = tahko.project_stat([], SPEC, LEAGUE, "2026-06-01")
    assert proj["rate"] == LEAGUE
    assert proj["effective_n"] == 0


def test_small_samples_shrink_harder():
    small = tahko.project_stat(_games([("2026-05-30", 8, 10)]), SPEC, LEAGUE, "2026-06-01")
    big = tahko.project_stat(_games([("2026-05-30", 400, 500)]), SPEC, LEAGUE, "2026-06-01")
...
def test_recent_games_outweigh_old_ones():
    ...
    recent = tahko.project_stat(hot_recent, SPEC, LEAGUE, "2026-06-01")["rate"]
    ancient = tahko.project_stat(hot_ancient, SPEC, LEAGUE, "2026-06-01")["rate"]
...
def test_projections_track_latent_talent_in_demo():
    ...
    for pid, p in players.items():
        proj = tahko.project_player(conn, pid, as_of="2026-09-01", league=league)
...
def test_aging_curve_shape():
    ...
    curve = tahko.aging_curve(conn, "kl_pct")
...
def test_fit_decay_returns_spec_from_grid():
    ...
    tuned = tahko.fit_decay(conn, SPEC, betas=(0.99, 0.999), strengths=(20.0, 100.0),
                            league_mean=LEAGUE)
